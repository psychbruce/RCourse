...
`set.seed()`设置的是**全局随机数生成器（RNG）的种子**，其影响范围是**从调用位置开始，到会话结束或下一次调用`set.seed()`为止的所有随机数生成操作**（“全局且持续到下一次显式更改”）。
...
-   利用假设检验的思想，判断这枚硬币是不是普通硬币（H0：硬币是普通的）

作业要求：

-   写一段**不超过3行**的R代码，计算并呈现连续1\~N次正面朝上的概率
-   根据程序运行结果，判断连续多少次正面朝上可以认定硬币不再是普通硬币（拒绝零假设），需要区分不同显著性水平下的结论（α = 0.05并不是“金标准”）
    -   *p* \< .05（很宽松）
    -   *p* \< .01（较宽松）
    -   *p* \< .001（较严格）
