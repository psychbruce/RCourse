...
[![Maple Mono SC NF 等宽字体效果（适合中英文代码混排）](images/clipboard-3688189296.png)](https://github.com/subframe7536/maple-font)
...
-   `func_xxx`：某个具体的R函数名称（比如`set.wd`、`set_wd`——设置工作目录路径）
    -   函数（以及任何R对象）的名称可以包含下划线（`_`）、点（`.`）等特殊字符，但不能有横线（`-`）、空格（）等其他字符
        -   `set.wd()`：可以这样命名
        -   `set_wd()`：可以这样命名
        -   `set-wd()`：不可以这样命名
        -   `set wd()`：不可以这样命名
-   `package::function()`调用方式（较常使用，建议掌握）
    -   在不加载（`library`）R包的情况下，临时使用其中的某个函数（当然R包需要已经安装在你的电脑里）
    -   如果来自不同R包的两个函数“撞名”了，就需要这样明确调用“哪个包的哪个函数”
-   `package:::function()`调用方式（极少使用，拓展了解即可）
