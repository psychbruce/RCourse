...
- R语言回归模型公式中的交互作用设定（适用于所有类型的回归模型）
  ...
  - `Y ~ 1 + A + B + A:B`（展开写法）
    - `1`为截距（可省略不写）
    - `A`和`B`为一阶效应（“简单效应”、“条件效应”），并非ANOVA中的“主效应”！！！⭐️
...
anova(m5)  # 回归模型各因素的F检验（Type I SS）：主效应、交互作用
...
- R语言内置`anova()`：“I型平方和”的*F*检验
  - 只适用于平衡样本情况，如果组间样本不平衡，则与“III型平方和”不一致
- 推荐使用`emmeans::joint_tests()`：“III型平方和”的*F*检验或卡方检验
  - 更通用的**主效应检验**函数！！！⭐️

```{r}
emmeans::joint_tests(m5)  # 联合对比检验（Type III SS）：真正的主效应、交互作用
summary(m5)  # A和B的简单效应（条件效应），并非“主效应”！！！
## A和B的简单效应（条件效应），并非“主效应”！！！
...
- 回归分析中，如果存在交互作用（A × B），则每个自变量的斜率代表另一个变量取0时，该变量的**简单斜率（一阶效应、简单效应、条件效应）**！
- 在【实践4】模型结果中：
...
- 变量中心化（[Chap05](https://psychbruce.github.io/RCourse/Chap05#%E5%8F%98%E9%87%8F%E4%B8%AD%E5%BF%83%E5%8C%96%E4%B8%8E%E6%A0%87%E5%87%86%E5%8C%96){.uri}）：使新变量（B'）取0等价于原变量（B）取均值
  - 中心化消除了“边际效应”与“条件效应”之间的偏差
    - 边际效应（主效应）= 条件效应（一阶斜率）+ 交互效应 × 调节变量均值
...
anova(m6)  # 回归模型各因素的F检验（Type I SS）：主效应、交互作用
emmeans::joint_tests(m6)  # 联合对比检验（Type III SS）：真正的主效应、交互作用
summary(m6)  # A和B中心化后，简单效应（条件效应）等价于“主效应”
## A和B中心化后，简单效应（条件效应）等价于“主效应”
...
MANOVA(d, dv="SCORE", between=c("A", "B"))
model = lm(SCORE ~ A * B, data=d)
summary(model)  # A和B的简单效应（条件效应），并非“主效应”！！！
anova(model)  # 回归模型各因素的F检验（Type I SS）：主效应、交互作用
emmeans::joint_tests(model)  # 联合对比检验（Type III SS）：真正的主效应、交互作用
```
