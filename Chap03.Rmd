...
```{r, eval=FALSE}
## 导出到剪切板，可粘贴（Ctrl + V）到其他地方
export(between.2)
```

```{r}
## 导出到CSV逗号分隔值纯文本文件
export(between.2, file="data.csv")
## 导出到SPSS数据文件
export(between.2, file="data.sav")
...
```{r}
## 保存为RData
save(d1, d2, d3, file="datasets1.RData")
...
![](images/clipboard-426724489.png)
...
- 开始准备个人期末大作业，自主选择任意一个感兴趣的公开数据，写出数据导入代码
  - 数据选择：可以从本课程提供的“[公开数据可选列表](https://docs.qq.com/sheet/DTXNCUWtzWWNuU05Y)”中，选择一篇感兴趣的研究，下载其公开数据中的一个数据文件（只选择一个有代表性的数据文件即可，无需使用研究中的全部数据文件）
